...
public class ExameDao extends Dao {
	...
	public List<Exame> getExames()
	{
		...
		try
		{
			...
			String sql = "SELECT exame.id_exame, tabela_exame.exame_nome, paciente.nome_paciente, DATE_FORMAT(exame.data_exame, '%d/%m/%Y') data_exame, status_exame.status_nome "
					+ "FROM exame, tabela_exame, paciente, status_exame "
					+ "WHERE tabela_exame.id_tabelaExame = exame.id_tabelaExame "
					+ "AND paciente.id_paciente = exame.id_paciente "
	...
	public Exame getExame(int idExame)
	{
		...
		try{			
		conn = getConnection();
		String sql = "SELECT exame.id_exame, exame.id_tabelaExame, exame.id_paciente, exame.descricao, exame.id_status, exame.nome_medico, exame.crm_medico , DATE_FORMAT(exame.data_exame, '%d/%m/%Y') data_exame, tabela_exame.exame_nome, status_exame.status_nome, paciente.cod_verif, paciente.nome_paciente"
		...
		if(result.next())
		{
			...
			exame.setNome_medico(result.getString("nome_medico"));
			exame.setCrm_medico(result.getString("crm_medico"));
...
public List<Exame> getResultado(int cod_verif)
	{
		...
		try
		{
			...
			String sql = "SELECT exame.id_exame, exame.id_paciente, exame.id_tabelaExame, DATE_FORMAT(exame.data_exame, '%d/%m/%Y') data_exame, tabela_exame.exame_nome, status_exame.id_status, status_exame.status_nome, paciente.nome_paciente "
			...
			while (result.next())
			{
				...
				stats.setId_status(result.getInt("id_status"));
...
public List<Exame> getHistorico(int idPaciente)
{
	...
	try
	{
		...
		String sql = "SELECT exame.id_exame, exame.id_paciente, exame.id_tabelaExame, DATE_FORMAT(exame.data_exame, '%d/%m/%Y') data_exame, exame.nome_medico, exame.crm_medico, tabela_exame.exame_nome, status_exame.status_nome "
				+ "FROM paciente, tabela_exame, status_exame, exame "
				+ "WHERE exame.id_paciente = paciente.id_paciente "
				+ "AND exame.id_tabelaExame = tabela_exame.id_tabelaExame "
				+ "AND exame.id_status = status_exame.id_status "
		...
		while (result.next())
		{
			...
			resultadoExame.setId_tabelaExame(result.getInt("id_tabelaExame"));
			resultadoExame.setData_exame(result.getString("data_exame"));
			resultadoExame.setNome_medico(result.getString("nome_medico"));
			resultadoExame.setCrm_medico(result.getString("crm_medico"));
...
public boolean insertExame(Exame exame)
{
	...
	try
	{
		...
		String sql = "insert into exame (id_paciente, id_tabelaExame, descricao, id_status, data_exame, nome_medico, crm_medico) values (?, ?, ?, ?, DATE_FORMAT(STR_TO_DATE(?, '%d/%m/%Y'), '%Y-%m-%d'), ?, ?);";
		...
		stmt.setString(6,  exame.getNome_medico());
		stmt.setString(7,  exame.getCrm_medico());
...
public boolean updateExame(Exame exame)
{
	...
	try
	{
		...
		String sql = "UPDATE exame SET id_tabelaExame=?, id_paciente=?, descricao=?, id_status=?, data_exame=DATE_FORMAT(STR_TO_DATE(?, '%d/%m/%Y'), '%Y-%m-%d'), nome_medico=?, crm_medico=? WHERE id_exame=?";        
