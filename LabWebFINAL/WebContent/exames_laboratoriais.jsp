 ...
 <script>
  function deletagem(x){
	  $( "#dialog-confirm" ).dialog({
	      ...
	      buttons: {
	        "Excluir": function() {
	         window.location.href = "DeletarExamelabServlet?exame="+x;	         
	        },
	        Cancel: function() {
	          $( this ).dialog( "close" );
	        }
...
<c:choose>
  ...
  <ul class="lista-exames">
    ...
    <li>      
      ...
      <c:if test="${loginUsuario.perfil.gestor || loginUsuario.perfil.admin}"> 
      <button class="but1" style="width:56px; height: 23px;" id="deletarexame" onclick="deletagem(${exame.id_tabelaExame})"></button>
...
<div style="display:none;" id="dialog-confirm" title="Excluir Resultado de Exame">
  			<p><span class="ui-icon ui-icon-alert" style="float:left; margin:0 7px 20px 0;"></span>Tem certeza que deseja excluir o exame/procedimento selecionado? Lembrando que caso exclua, todos os dados deste exame serão perdidos permanentemente</p>
	</div>
