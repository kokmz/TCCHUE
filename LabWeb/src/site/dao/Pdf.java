...
public class Pdf extends Dao {
	...
	public void GerarPdf(java.util.List<FluxoExames> fluxo) throws Exception {
		...
		try {
			...
			os = new FileOutputStream(localPath + "/Desktop/Relatorio-"+new SimpleDateFormat("ddMMyyyy").format(new Date())+"-"+new SimpleDateFormat("hhmmss").format(new Date())+".pdf");
			...
			String caminhoAtual = new File("banner.jpg").toString();
	
			
		

			Image img = Image.getInstance(caminhoAtual);
