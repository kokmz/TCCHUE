...
public class FluxoDeExamesServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		if (data_fim.isEmpty() || data_inicio.isEmpty()) {
			...
			getServletContext().getRequestDispatcher("/fluxoExames.jsp")
					.forward(request, response);
		} else {
			ConsultaDao ConsultaDao = new ConsultaDao();
			List<FluxoExames> fluxo = ConsultaDao.buscaExame(data_inicio,
					data_fim);

			request.setAttribute("fluxo", fluxo);
			getServletContext().getRequestDispatcher("/consultaFluxo.jsp")
					.forward(request, response);
