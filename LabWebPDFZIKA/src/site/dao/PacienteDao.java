...
public class PacienteDao extends Dao {
	...
	public Paciente getPaciente(int idPaciente)
	{
		...
		try
		{
			...
			String sql = "SELECT p.id_paciente, p.cod_verif, p.nome_paciente, p.rg_paciente, p.cpf_paciente, p.datanasc_paciente, p.cidade_endereco, p.numero_endereco, p.complemento_endereco, p.telefone_paciente, p.email_paciente, p.convenio_paciente, p.id_logradouro, l.id_logradouro, l.cep, l.endereco, l.bairro, e.id_estado, e.sigla_estado, e.nome_estado FROM paciente p, logradouro l, estado e WHERE p.id_logradouro = l.id_logradouro AND p.id_estado = e.id_estado AND p.id_paciente=?";        
			...
			if (result.next())
			{
				...
				paciente.setId_logradouro(result.getInt("id_logradouro"));
	...
	public List<Paciente> getPacientes()
	{
		...
		try
		{
			...
			String sql = "SELECT p.id_paciente, p.cod_verif, p.nome_paciente, p.rg_paciente, p.cpf_paciente, p.datanasc_paciente, p.cidade_endereco, p.numero_endereco, p.complemento_endereco, p.telefone_paciente, p.email_paciente, p.convenio_paciente, l.id_logradouro, l.cep, l.endereco, l.bairro, e.id_estado, e.sigla_estado FROM paciente p, logradouro l, estado e WHERE p.id_logradouro = l.id_logradouro AND p.id_estado = e.id_estado";        
	...
	public boolean insertPaciente(Paciente paciente)
	{
		...
		try
		{
			...
			String sql = "insert into paciente (cod_verif, nome_paciente, rg_paciente, cpf_paciente, datanasc_paciente, id_logradouro, cidade_endereco, numero_endereco, complemento_endereco, id_estado, telefone_paciente, email_paciente, convenio_paciente) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";        
			...
			stmt.setInt(6, paciente.getId_logradouro());
			stmt.setString(7, paciente.getCidade_paciente());
			stmt.setInt(8, paciente.getNumero_endereco());
	...
	public boolean updatePaciente(Paciente paciente)
	{
		...
		try
		{
			...
			String sql = "UPDATE paciente SET cod_verif=?, nome_paciente=?, rg_paciente=?, cpf_paciente=?, datanasc_paciente=?, id_logradouro=?, cidade_endereco=?, numero_endereco=?, complemento_endereco=?, id_estado=? , telefone_paciente=?, email_paciente=?, convenio_paciente=? WHERE id_paciente=?";        
